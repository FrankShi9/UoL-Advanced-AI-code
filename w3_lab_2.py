...
def findInformationGain(data, label, column, entropyParent):
    keys = { i[column] for i in data } # set
    entropies = {}                     # set
    count = {}                         # set
    avgEntropy = 0
    for val in keys:
...
def makeDecisionTree(data, label, parent=-1, branch=''):
    ...
    if parent >= 0:
        edges.append((parent, node, branch)) # a tuple

    # Find the variable (the column) with max info gain
    infoGain = []
    ...
    #Create a leaf node if max info gain is not significant
    ...
    parent = node
    node += 1
    # All out-going edges from current node
    branches = { i[splitColumn] for i in data }
