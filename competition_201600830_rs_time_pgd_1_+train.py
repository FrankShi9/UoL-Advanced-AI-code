...
def pgd_whitebox(model, X, y, restarts=3, epsilon=args.epsilon, num_steps=args.num_steps, step_size=args.step_size):
    ...
    j = 0
    # restart
    # while j < restarts and timecost_now < 0.08: # attack
    while j < restarts: # train
        # random start
        if args.random:
            random_noise = torch.FloatTensor(*X_pgd.shape).uniform_(-epsilon, epsilon).to(device)
            X_pgd = Variable(X_pgd.data + random_noise, requires_grad=True)
        i = 0
        while i < num_steps: # steps per pgd loop
            ...
            i += 1
        # print('pgd steps per restart:', i, end='  ')
        j += 1
    ...
    #debug
    # print('tc: ', timecost_now, end='  ')
    # print('restart number:', j, end='\n')
